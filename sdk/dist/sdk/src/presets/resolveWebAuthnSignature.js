...
export const resolveWebAuthnSignature = (loginUrl, loginUsername) => async (ctx) => {
...
const signUserOp = async (userOpHash, passkeyIdHex) => {
    ...
    const challengePrefix = challengeOffsetRegex.exec(base64url
...
const signUserOpWithCreate = async (userOpHash, login, loginUrl) => {
    console.log({ userOpHash });
    const challenge = Buffer.from(userOpHash.slice(2), "hex");
    ...
    const pubKeyCoordinates = [
        "0x" +
            base64url
                .toBuffer(decodedPassKey.response.attestationObject.authData
    ...
    if (supportsDirectAttestation) {
        const ecVerifyInputs = authResponseToSigVerificationInput(decodedPassKey.response.attestationObject.authData
        ...
        const challengePrefix = challengeOffsetRegex.exec(base64url.toBuffer(passkey.response.clientDataJSON).toString("hex"))?.[1];
        console.log({ challengeOffsetRegex, challengePrefix });
        console.log("webauthn verify inputs", [
    ...
    console.log("login service inputs", ethers.utils.defaultAbiCoder.decode(["bytes1", "string", "bytes", "uint256[2]", "bytes"], loginServiceData));
    return loginServiceData.signature;
};
var SignatureTypes;
(function (SignatureTypes) {
    SignatureTypes[SignatureTypes["NONE"] = 0] = "NONE";
    SignatureTypes[SignatureTypes["WEBAUTHN_UNPACKED"] = 1] = "WEBAUTHN_UNPACKED";
    SignatureTypes[SignatureTypes["LOGIN_SERVICE"] = 2] = "LOGIN_SERVICE";
    SignatureTypes[SignatureTypes["WEBAUTHN_UNPACKED_WITH_LOGIN_SERVICE"] = 3] = "WEBAUTHN_UNPACKED_WITH_LOGIN_SERVICE";
})(SignatureTypes || (SignatureTypes = {}));
//# sourceMappingURL=resolveWebAuthnSignature.js.map