...
import { BigNumber, BigNumberish, Signer, constants, ethers } from "ethers";
...
import { RpcError, requireCond, tostr } from "./utils";
...
export class UserOpMethodHandler {
  ...
  async estimateUserOperationGas(
    ...
    console.log({
      ...
      data: this.entryPoint.interface.encodeFunctionData("simulateHandleOp", [
        userOp,
    ...
    let callGasLimit;
    if (userOp.initCode?.length > 2 && userOp.callData?.length > 2) {
    ...
    console.log({
      callGasLimit: ethers.BigNumber.from(callGasLimit).toString(),
    });
